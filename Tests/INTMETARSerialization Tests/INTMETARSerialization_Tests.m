...
- (void)testRVR
    {
        NSArray *a = @[
                       ...
                       @"KGFK 032253Z 35023KT R34L/4500V6000FT 10SM OVC055 M04/M06 A2920 RMK AO2 T10441061",
                       ];
        // We should really have a test that allows for checking multiple RVRs as the property is an array.
        NSArray *ex = @[@"R35L/4500V6000FT", @"R34L/4500V6000FT"];
        [a enumerateObjectsUsingBlock:^(NSString *s, NSUInteger idx, BOOL *stop) {
            NSError *e = nil;
            INTMETARSerialization *m = [INTMETARSerialization METARObjectFromString:s options:INTMETARParseOptionStrict|INTMETARParseOptionLogWarnings error:&e];
            if (e) {
            ...
            NSString *expected = [ex objectAtIndex:idx];
            if ([expected isEqualToString:m.runwayVisualRanges.firstObject]) {
                XCTFail(@"Expecting RVR %@, found RVR %@ in %@", expected, m.runwayVisualRanges.firstObject, m.metarString);
            }
        }];
    }
...
- (void)testWeatherPhenomena
    {
        ...
        // TODO: Allow testing for multiple weather phenomena at a time.
        ...
        [a enumerateObjectsUsingBlock:^(NSString *s, NSUInteger idx, BOOL *stop) {
            ...
            NSString *expected = [ex objectAtIndex:idx];
            if (![expected isEqualToString:m.weatherPhenomena.firstObject]) {
                XCTFail(@"Expecting phenomena %@, found phenomena %@ in %@", expected, m.weatherPhenomena.firstObject, m.metarString);
            }
        }];
    }
...
- (void)testSkyConditions
    {
        ...
        // TODO: Allow testing for multiple sky conditions at a time.
        NSArray *ex = @[@"BKN035", @"SCT029", @"BKN011"];
        [a enumerateObjectsUsingBlock:^(NSString *s, NSUInteger idx, BOOL *stop) {
            NSError *e = nil;
            INTMETARSerialization * m =[INTMETARSerialization METARObjectFromString:s options:INTMETARParseOptionStrict|INTMETARParseOptionLogWarnings error:&e];
            if (e) {
            ...
            NSString *expected = [ex objectAtIndex:idx];
            if (![expected isEqualToString:m.skyConditions.firstObject]) {
                XCTFail(@"Expecting sky condition %@, found sky condition %@ in %@", expected, m.skyConditions.firstObject, m.metarString);
            }
        }];
    }
